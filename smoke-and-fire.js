...
function setup() {
  ...
  geo.load2DGrid(
    ...
    function(progress) {
      loadingbar.update(0, (progress.loaded / 2955310) * 100);
    }
  );

  geo.load2DGrid(
    "data/data-nsw-3k-PM25.json",
    function(object) {
      ...
      var boundary = geoutils.makeBoundary(
        nsw_pollution_3k_PM25.data[0].boundary
      );
      boundary.visible = false;
      data_boundaries.push(boundary);
      renderer.add(boundary);
    ...
    function(progress) {
      loadingbar.update(1, (progress.loaded / 2966131) * 100);
    }
  );

  geo.load2DGrid(
    "data/data-nsw-1k-PM25.json",
    function(object) {
      ...
      var boundary = geoutils.makeBoundary(
        nsw_pollution_1k_PM25.data[0].boundary
      );
      boundary.visible = false;
      data_boundaries.push(boundary);
      renderer.add(boundary);
    ...
    function(progress) {
      loadingbar.update(2, (progress.loaded / 3010005) * 100);
    }
  );
  ...
  geo.load2DGrid(
    ...
    function(progress) {
      loadingbar.update(3, (progress.loaded / 2508951) * 100);
    }
  );

  geo.load2DGrid(
    "data/data-nsw-3k-shipping.json",
    function(object) {
    ...
    function(progress) {
      loadingbar.update(4, (progress.loaded / 3157944) * 100);
    }
  );

  geo.load2DGrid(
    "data/data-nsw-1k-shipping.json",
    function(object) {
    ...
    function(progress) {
      loadingbar.update(5, (progress.loaded / 3320845) * 100);
    }
  );

  geo.loadPoints(
    "data/data-pointsources-shipping.json",
    function(object) {
    ...
    function(progress) {
      loadingbar.update(6, (progress.loaded / 521395) * 100);
    }
  );

  geo.loadPoints(
    "data/data-aus-pointsources.json",
    function(object) {
      ...
      aus_point_emissions.mesh.visible = true;
      renderer.add(aus_point_emissions.mesh);
      loadingbar.complete(7);
    },
    function(progress) {
      loadingbar.update(7, (progress.loaded / 44250) * 100);
    }
  );

  geo.loadPoints(
    "data/data-pointsources-3k.json",
    function(object) {
    ...
    function(progress) {
      loadingbar.update(8, (progress.loaded / 944791) * 100);
    }
  );
} // end setup
...
function update() {
  ...
  if (aus_pollution_PM25.ready) {
    ...
    if (aus_pollution_PM25.data[i].timestamp) {
      ...
      GUI_time.innerHTML = moment(aus_pollution_PM25.data[i].timestamp).format(
        "LLLL"
      );
    }
  }
...
var GUI_tool_plume_min_display = document.getElementById(
  "tool-plume-min-display"
);
...
GUI_tool_plume_min.addEventListener("input", function(event) {
  if (aus_shipping_PM25.ready) {
    aus_shipping_PM25.material.uniforms.minValue.value = Number(
      event.target.value
    );
    nsw_shipping_3k_PM25.material.uniforms.minValue.value = Number(
      event.target.value
    );
    nsw_shipping_1k_PM25.material.uniforms.minValue.value = Number(
      event.target.value
    );
...
GUI_tool_plume.addEventListener("input", function(event) {
  if (aus_shipping_PM25.ready) {
    aus_shipping_PM25.material.uniforms.normalValue.value = Number(
      event.target.value
    );
    nsw_shipping_3k_PM25.material.uniforms.normalValue.value = Number(
      event.target.value
    );
    nsw_shipping_1k_PM25.material.uniforms.normalValue.value = Number(
      event.target.value
    );
...
var GUI_tool_smoke_min_display = document.getElementById(
  "tool-smoke-min-display"
);
...
GUI_tool_smoke_min.addEventListener("input", function(event) {
  if (aus_pollution_PM25.ready) {
    aus_pollution_PM25.material.uniforms.minValue.value = Number(
      event.target.value
    );
...
GUI_tool_smoke.addEventListener("input", function(event) {
  if (aus_pollution_PM25.ready) {
    aus_pollution_PM25.material.uniforms.normalValue.value = Number(
      event.target.value
    );
    nsw_pollution_3k_PM25.material.uniforms.normalValue.value = Number(
      event.target.value
    );
    nsw_pollution_1k_PM25.material.uniforms.normalValue.value = Number(
      event.target.value
    );
